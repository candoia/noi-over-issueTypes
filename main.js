...
$(window).load(function() {
  ...
  let json = api.boa.run('NOI_per_IssueType.boa');
  $('#loading').hide();
  $('#content').show();
  ...
  for(let index in json.issuesCount) {
    count++;
    $('#numToShow').append(`<option value="${count}"> ${count} </option>`);
    chartData.push({
        label: index,
        value: json.issuesCount[index],
  ...
  $('#app-title').html(`Number of Issues per Issue Type for Project ${instance.repos.name}`);
